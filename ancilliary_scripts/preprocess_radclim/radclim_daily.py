...
while current_timestamp <= end_timestamp:
    ...
    filenamezip =  os.path.join(directory,'mnt/HDS_RADCLIM/RADCLIM/export',year,month,day,'%s%s%s.radclim.1h.hdf.zip'%(year, month, day))
...
#       zip_ref.extractall(directory)
       ...
       for member in zip_ref.namelist():
         ...
         target_path = os.path.join(directory,'mnt/HDS_RADCLIM/RADCLIM/export',year,month,day, filename)
    ...
    file_path = os.path.join(directory,'mnt/HDS_RADCLIM/RADCLIM/export',year,month,day, filename)
    ...
    if (hour == "23"):  
      ...
      netcdf_file_path = os.path.join(output_directory,'%s%s%s.radclim.1h.nc'%(year, month, day))
      ...
      lat_bounds_var = ncfile.createVariable('lat_bounds', np.float32, ('nlat', 'nlon', 'nv'))
      lon_bounds_var = ncfile.createVariable('lon_bounds', np.float32, ('nlat', 'nlon', 'nv'))
      times = ncfile.createVariable('time', 'f8', ('time',))
      precipitation = ncfile.createVariable('precipitation', 'f4', ('time', 'nlat', 'nlon',), fill_value=-9999.)    
      ...
      # Assigning time data
      dt_format = "%Y%m%d%H%M%S"
      times_list = [datetime.datetime.strptime(ts, dt_format) for ts in time_info_list]
      ...
      precipitation.units = 'mm'  # Assuming radar data is precipitation; adjust as needed
