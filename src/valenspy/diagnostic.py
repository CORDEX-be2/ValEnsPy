    ...
    def visualize(self, result, axes=None, facetted=True, **kwargs):
        ...
        if axes is None:
            if facetted:
                ...
                ax = plt.gca()
        return self.visualization_function(result, axes=axes, facetted=facetted, **kwargs)
        ...
        def diagnostic_function(dt: DataTree, ref, **kwargs):
            ...
            if isinstance(ref, DataTree):
                for data_node, ref_node in zip(dt.leaves, ref.leaves):
                    ensemble_results[data_node.path] = model2ref.diagnostic_function(data_node.ds, ref_node.ds, **kwargs)
            else:
                for data_node in dt.leaves:
                    ensemble_results[data_node.path] = model2ref.diagnostic_function(data_node.ds, ref, **kwargs)
            return ensemble_results

        def visualization_function(results, axes, facetted=facetted, **kwargs):
            if facetted:
                for path, result, ax in zip(results.keys(), results.values(), axes.flatten()):
                    model2ref.visualize(result, ax=ax, **kwargs)
                    ax.set_title(path.replace("/", " "))
            else:
                for path, result in results.items():
                    model2ref.visualize(result, ax=axes, label=f'{path.replace("/", " ")}', **kwargs)
            return axes

        return Ensemble2Ref(diagnostic_function, visualization_function, model2ref.name, model2ref.description)